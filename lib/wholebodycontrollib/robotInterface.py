...
class robotInterface():
    ...
    def __init__(self, robot_name, local_port_prefix, axis_list, remote_control_board_list) -> None:
    ...
    def open(self) -> bool:
        ...
        yarp.Network.init()
        if not yarp.Network.isNetworkInitialized() or not yarp.Network.checkNetwork(5.0):
    ...
    def __init_interfaces(self):
        ...
        self.iPosDirectCtrl = self.robotDriver.viewIPositionDirect()
    ...
    def __set_joints_control_modes(self, joint_list, control_mode) -> bool:
        for j in joint_list:
            if not self.iCtrlMode.setControlMode(j, control_mode):
                return False
        return True
    ...
    @staticmethod
    def to_yarp_DVector(numpy_array) -> yarp.DVector:
        buffer = yarp.DVector()
    ...
    @staticmethod
    def to_yarp_IVector(numpy_array) -> yarp.IVector:
        buffer = yarp.IVector()
        buffer.reserve(numpy_array.size)
        _ = [buffer.append(int(val)) for val in numpy_array]
        return buffer
    ...
    def set_position_control_mode(self, joint_list = None):
        if joint_list is None:
            return self.__set_all_control_modes(yarp.VOCAB_CM_POSITION)
        else:
            return self.__set_joints_control_modes(joint_list, yarp.VOCAB_CM_POSITION)
        
    def set_position_direct_control_mode(self, joint_list = None):
        if joint_list is None:
            return self.__set_all_control_modes(yarp.VOCAB_CM_POSITION_DIRECT)
        else:
            return self.__set_joints_control_modes(joint_list, yarp.VOCAB_CM_POSITION_DIRECT)
    
    def set_torque_control_mode(self, joint_list = None):
        if joint_list is None:
           return self.__set_all_control_modes(yarp.VOCAB_CM_TORQUE)
        else:
            return self.__set_joints_control_modes(joint_list, yarp.VOCAB_CM_TORQUE)

    def move(self, positions: np.ndarray) -> bool:
        if positions.size != self.ndof:
            raise ValueError('Failed to set move position.')
        
        return self.iPosCtrl.positionMove(self.to_yarp_DVector(np.rad2deg(positions)))
    ...
    def set_joints_position(self, positions: np.ndarray, joint_list = None) -> bool:
        if joint_list is None:
            if positions.size != self.ndof:
                raise ValueError('Failed to set joint positions.')
            return self.iPosDirectCtrl.setPositions(self.to_yarp_DVector(np.rad2deg(positions)))
        else:
            return self.iPosDirectCtrl.setPositions(len(joint_list), self.to_yarp_IVector(joint_list), self.to_yarp_DVector(np.rad2deg(positions)))

    def set_joints_torque(self, torques: np.ndarray, joint_list = None) -> bool:
        if joint_list is None:
            if torques.size != self.ndof:
                raise ValueError('Failed to set joint torques.')
            return self.iTrqCtrl.setRefTorques(self.to_yarp_DVector(torques))
        else:
            return self.iTrqCtrl.setRefTorques(len(joint_list), self.to_yarp_IVector(joint_list), self.to_yarp_DVector(torques))
